...
int CryptoService::encrypt_text(const std::array<uint8_t, 32>& key,
...
int CryptoService::generate_nonce(std::array<uint8_t, 12>& out_nonce) {
    ...
    rc = mbedtls_ctr_drbg_random(&ctr_drbg, out_nonce.data(), out_nonce.size());
...
int CryptoService::decrypt_text(const uint8_t key[32],
