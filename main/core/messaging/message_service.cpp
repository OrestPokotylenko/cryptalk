...
namespace cryptalk::messaging  {    
    int MessageService::generate_message(
        ...
        int version = 1;
        std::vector<uint8_t> aad = build_message_aad(version, type, flag, message_id, device_id, receiver_id);

        std::array<uint8_t, 16> tag_out = {};
        std::array<uint8_t, 12> nonce_out = {};
        std::vector<uint8_t> ciphertext_out(plaintext_len);

        int rc = crypto_service.encrypt_text(
            key,
            plaintext,
            plaintext_len,
            aad.data(),
