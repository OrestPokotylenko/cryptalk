...
static const std::array<uint8_t, 32> TEST_KEY = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
...
static std::vector<uint8_t> make_test_aad() {
    ...
    return cryptalk::messaging::build_message_aad(1, 2, 3, 0x11223344, sender_id, receiver_id);
}
...
static void test_encrypt_decrypt_roundtrip() {
    ...
    int enc_rc = service.encrypt_text(TEST_KEY,
                                      plaintext,
                                      plaintext_len,
                                      aad.data(),
                                      aad.size(),
    ...
    int dec_rc = service.decrypt_text(TEST_KEY.data(),
                                      ciphertext_out.data(),
                                      ciphertext_out.size(),
                                      nonce_out.data(),
                                      aad.data(),
